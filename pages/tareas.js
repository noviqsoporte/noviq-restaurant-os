...
export default function Tareas() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const [tareasRes, usersRes] = await Promise.all([
        fetch('/api/tareas'),
        fetch('/api/usuarios'),
      ]);
  ...
  const toggleResponsable = (userId) => {
    setForm((f) => {
      ...
      if (current.includes(userId)) {
        return { ...f, Responsable: current.filter((id) => id !== userId) };
      }
      return { ...f, Responsable: [...current, userId] };
    });
  };
    ...
    <>
                ...
                data.map((r) => (
                  <tr key={r.id} className={isOverdue(r) ? 'border-l-2 border-l-red-500' : ''}>
                    ...
                    <td className="text-gray-500 font-mono text-xs">
                      {r['ID Telegram (from Responsable)']
                        ? (Array.isArray(r['ID Telegram (from Responsable)'])
                          ? r['ID Telegram (from Responsable)'].join(', ')
                          : r['ID Telegram (from Responsable)'])
                        : '—'}
                    </td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editingId ? 'Editar Tarea' : 'Nueva Tarea'}>
        <div className="space-y-4">
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Tarea</label>
            <input className="input-field" value={form.Tarea} onChange={(e) => u('Tarea', e.target.value)} placeholder="Nombre de la tarea" />
          </div>
          <div>
            <label className="text-xs text-gray-400 mb-1 block">Descripción</label>
            <textarea className="input-field" rows={2} value={form['Descripción']} onChange={(e) => u('Descripción', e.target.value)} placeholder="Detalle de la tarea..." />
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {usuarios.map((usr) => (
                <button
                  key={usr.id}
                  type="button"
                  onClick={() => toggleResponsable(usr.id)}
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium border transition-all ${
                    (form.Responsable || []).includes(usr.id)
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" className="input-field" value={form['Fecha Limite']} onChange={(e) => u('Fecha Limite', e.target.value)} />
            </div>
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Fecha de finalización</label>
              <input type="date" className="input-field" value={form['Fecha de finalización']} onChange={(e) => u('Fecha de finalización', e.target.value)} />
            </div>
          </div>
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              <label className="text-xs text-gray-400 mb-1 block">Activa</label>
